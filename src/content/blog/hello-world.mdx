...
「こんなのはどうだろう、面白そうだな。便利そうだな。」という考えが浮かんでもメモに残さず忘れたり、残しても溜まる一方で何もしないということを繰り返してきたので、今度こそ記録のために残そうという取り組みである。
...
・**Node.jsを使わずBunで開発環境を統一**している点。

パッケージのインストールやビルドが高速で、開発体験が向上した。

・CIの実行環境を用意せず、開発環境でセルフCIスクリプトを作成し実行するようにした。

これはプライベートリポジトリで色々と遊んでいた結果、GitHub Actionsの無料枠を超過したためである。。。

## 開発の過程でClaude Codeをどのように利用したか

最初は「GitHub Pagesで技術ブログを作りたい」という漠然とした要望から、以下のような過程を経て完成に至った：
...
- Astroのテンプレートから始め、不要な機能を削ぎ落としていった
- 「ミニマルなデザイン・機能にしたい」という要望に対し、Claude Codeが段階的にコンポーネントを簡素化

### 2. ローカルでのCI/CDの実行環境の整備

途中でGitHub Actionsの無料枠を超過したため、ローカル環境でCIを実行する仕組みを構築：
...
Claude Codeが提案した候補から「**Code & Living**」を選択：
- Tech Craft Lab
- Simple Dev Notes
- Build & Learn
- Dev Minimalist
...
技術と日常を繋ぐというネーミングから採用。あと調べた限り商標として名前が使用されていなかったので。(どうでも良い)
...
Playwright MCPのインストールと利用例：

```bash
# Playwright MCPのインストール（Claude Code内で実行）
mcp install @mcp/playwright
